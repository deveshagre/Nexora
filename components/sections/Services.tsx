  ...
  return (
    <section id="services" className="px-5 py-12 sm:py-16">
      <div className="mx-auto max-w-6xl">
        <h2 className="text-3xl font-black text-nexore-text">Our Services</h2>
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {services.map((service) => {
            const Icon = service.icon;
            return (
              <Card key={service.title} className="p-5">
                <span className="grid h-11 w-11 place-items-center rounded-xl bg-nexore-primary/22 text-nexore-accent">
                  <Icon className="h-5 w-5" />
                </span>
                <h3 className="mt-5 text-lg font-black text-nexore-text">{service.title}</h3>
